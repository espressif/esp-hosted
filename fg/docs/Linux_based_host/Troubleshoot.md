...
### 1.2 Unknown symbol error while executing kmod_build_and_load.sh
...
Run below command before execution of kmod_build_and_load.sh
```
sudo modprobe sdhci
```
...
On older Raspberry Pi OS (before March 2024), the GPIO numbers used for the `resetpin` parameter in `kmod_build_and_load.sh` and assigned to `HANDSHAKE_PIN` and `SPI_DATA_READY_PIN` in `esp_spi.h` should match the actual Raspberry Pi GPIOs on the header.
