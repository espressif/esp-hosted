...
$ ./esp_kmod_up.sh wifi=sdio bt=sdio resetpin=6
```
...
Once built, the kernel module `esp32_sdio.ko` can be found in `esp_hosted_fg/host/linux/kmod`. You can manualy load/unload the module as needed.

To add the module:

`$ sudo insmod esp_hosted_fg/host/linux/kmod/esp32_sdio.ko resetpin=518 clockspeed=50`
...
```
For SDIO, <esp_chipset> could be `esp32` or `esp32c6` or `esp32c5`
