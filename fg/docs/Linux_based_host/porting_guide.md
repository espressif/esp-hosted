...
- HCI in older kernel versions
	- HCI had compiled, but not tested for kernel version 3.18.16. Please refer [esp_bt_api.h](../../host/linux/kmod/esp_bt_api.h) for details.
	- In Linux kernel, BT over SPI *i.e.* symbol *HCI_SPI* is supported over kernel version 4.7.0. Please refer [esp_bt.c](../../host/linux/kmod/esp_bt.c). Ideally there should be no change required here.

## 2.2 kmod_build_and_load.sh
* [kmod_build_and_load.sh](../../host/linux/scripts/kmod_build_and_load.sh) is utility script to load the ESP kernel module
...
* Kernel Module  
	* Within [kmod_build_and_load.sh](../../host/linux/scripts/kmod_build_and_load.sh), make command used is Raspberry Pi specific. Following tags in `make` command should be changed as per platform
...
Test application is located at `host/linux/user_space/c_demo_app`.
...
* Disable default SPI driver
	- Linux kernel has a default SPI controller driver which needs to be disabled in order to make esp32_spi module work. Please see following code snippet from kmod_build_and_load.sh script
...
* Additional GPIOs
	...
	- Alter `HANDSHAKE_PIN` and `SPI_DATA_READY_PIN` in [esp_spi.h](../../host/linux/kmod/spi/esp_spi.h).
	- **Note:** See [Section 2.4.1](#241-gpio-numbering-in-raspberry-pi-os) for changes in Raspberry Pi OS GPIO mappings.
...
* SPI Bus instance and Chip select number
	...
	- It could be changed using variables, `esp_board.bus_num` and `esp_board.chip_select` in function `spi_dev_init()` from file `host/linux/kmod/spi/esp_spi.c`
* SPI mode
