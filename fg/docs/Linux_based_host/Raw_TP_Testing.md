...
- On Host side:
      1. While setting up the host, pass `rawtp` argument to `kmod_build_and_load.sh`.
          - e.g if you are setting Wi-Fi over SDIO and you want to test raw TP over SDIO in
          ```sh
          $ cd esp_hosted_fg/host/linux/scripts
          $ ./esp_kmod_up.sh wifi=sdio rawtp
        ```
...
To change the clock frequency, add the `clockspeed` parameter when loading the kernel module:

```sh
$ ../esp_kmod_up.sh wifi=sdio clockspeed=50 rawtp
```
