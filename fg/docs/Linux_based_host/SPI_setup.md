...
$ ./esp_kmod_up.sh wifi=spi bt=spi resetpin=6 spi_mode=3
```
...
Execute `./esp_kmod_up.sh --help` to see the list of options.
...
Once built, the kernel module `esp32_spi.ko` can be found in `esp_hosted_fg/host/linux/kmod`. You can manualy load/unload the module as needed.

To add the module:

`$ sudo insmod esp_hosted_fg/host/linux/kmod/esp32_spi.ko resetpin=518 clockspeed=10 spi_bus=0 spi_cs=0 spi_mode=3 spi_handshake=517 spi_dataready=524`
...
#### 2.2.1 Choose the Right Coprocessor Example

Before building firmware, select the appropriate coprocessor example based on your requirements:

| Use Case | Recommended Example | Path |
|----------|-------------------|------|
| **WiFi Only** | Minimal WiFi | `coprocessor/examples/minimal/wifi/` |
| **Bluetooth Only** | Minimal Bluetooth | `coprocessor/examples/minimal/bt/` |
| **WiFi + Custom Communication** | Custom RPC | `coprocessor/examples/extensions/custom_rpc_msg/` |
| **WiFi + Advanced Networking** | Network Split | `coprocessor/examples/extensions/network_split/` |

**Recommendation**: Start with `examples/minimal/wifi` for basic WiFi functionality, then explore extensions as needed.
...
##### Set-up ESP-IDF
- **Note on Windows 11**: follow [these instructions](../../coprocessor/sdk_esp_idf_setup/setup_windows11.md) to setup ESP-IDF and build the esp firmware.
...
* Navigate to your chosen example directory from the table above
```
$ cd <chosen_example_path>
$ rm -rf sdkconfig build
$ idf.py set-target <esp_chipset>
```
Example for minimal WiFi:
```
$ cd coprocessor/examples/minimal/wifi
$ rm -rf sdkconfig build
