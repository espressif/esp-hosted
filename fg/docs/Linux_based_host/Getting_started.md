...
* Ensure that `hci0` interface is visible. To check that, run `sudo bluetoothctl` and fire `list` to see the adapter.

Although `hciconfig` is deprecated, it it available for your BlueZ version, you can alternatively run `hciconfig` to see,
```
...
Run `sudo bluetoothctl` shell and fire `scan on` for BT device scanning. Use `scan off` to stop scanning.

#### 3.1.4 BLE scan

Run `sudo bluetoothctl` shell and fire `scan on` for BLE device scanning. Use `scan off` to stop scanning.
...
##### Using 2M phy:
2M phy cannot be used for initial connection in BLE5.X. Configure phy as 1M and 2M both, make connection with other BLE5.X device and then set phy as 2M.

Steps:
1. To configure and check phy settings, use `sudo btmgmt --index hci0 phy` to view current settings.
2. To set 2M phy preferences, use `sudo btmgmt --index hci0 phy-configuration 1M 2M`.
3. Connect to BLE5.X device using steps in section 3.2.1.
4. After connection, the PHY will be negotiated automatically based on both devices' capabilities.
5. Follow gatt read/write from `menu gatt` in bluetoothctl.

##### Using CODED phy:
Configure CODED phy on host and peripheral side.

Steps:
1. To configure CODED phy, use `sudo btmgmt --index hci0 phy-configuration CODED`.
2. To check selected phy, run `sudo btmgmt --index hci0 phy`.
3. Connect to BLE5.X device using steps in section 3.2.1.
