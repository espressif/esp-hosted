...
$ ./esp_kmod_up.sh wifi=<transport> bt=<bt_over_uart> resetpin=6
```
	- <transport> can take value `sdio` or `spi`. Defaults to `sdio`
	- <bt_over_uart> can take value `uart_4pins` or `uart_2pins`.
	  - This option will setup HCI over UART
	  - :warning: Note:
	    - For ESP32-C2/C6, 2 pin UART (TX & RX only) is supported. So `uart_2pins` should be used for ESP32-C2/C6.
		- For other chipsets, `uart_4pins` should be used, where 4 pin UART (TX, RX, CTS, RTS) is used.

### 2.2 ESP Peripheral Firmware
UART is only supporting dedicated lines for Bluetooth in esp_hosted_fg. Supported example is `coprocessor/examples/minimal/bt/`. This example demonstrates combined SPI/SDIO+UART. SPI/SDIO provides serial interface. This interface can be used for RPC for controlling co-processor from host or it can also carry Wi-Fi load multiplexed. Bluetooth traffic only is offloaded through UART using standard HCI.

> [!NOTE]
>
> If you don't need RPC communication between the coprocessor and host, modify `app_main.c` in the example to
> Replace line,
> ```c
> esp_hosted_cp_lin_fg_config_t config = ESP_HOSTED_CP_LIN_FG_CALLBACK_BASED_CONFIG();
> ESP_ERROR_CHECK(esp_hosted_cp_lin_fg_init(&config));
> ```
> with
> ```c
> esp_hosted_cp_lin_fg_config_t config = ESP_HOSTED_CP_LIN_FG_NO_RPC_CONFIG();
> ESP_ERROR_CHECK(esp_hosted_cp_lin_fg_init(&config));
> ```
