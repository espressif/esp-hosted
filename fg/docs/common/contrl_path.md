...
- Code for **MPU based** serial driver is at [platform_wrapper.h](../../host/components/esp_hosted_rpc_lib/port/linux/include/platform_wrapper.h)
...
* The Demo application is at the top, which uses [control path APIs](ctrl_apis.md) exposed by `esp_hosted_rpc_lib` (Hosted control library). The `esp_hosted_rpc_lib` in turn uses the virtual serial interface exposed by `Host serial driver`
...
There are two components in control path. The `esp_hosted_rpc_lib` (Hosted control lib) and Demo app. Demo App uses control APIs to communicate to the `esp_hosted_rpc_lib`.

###### 3.1.1 esp_hosted_rpc_lib (Hosted Control Lib)
- The `esp_hosted_rpc_lib` (Hosted Control Library) is available as base library which exposes control APIs so that user application could be easily integrated
- It is responsible for Protobuf encoding and decoding
- Control path APIs are exposed in [ctrl_api.h](../../host/components/esp_hosted_rpc_lib/include/ctrl_api.h) file
- Control path library is implemented with source files, [ctrl_api.c](../../host/components/esp_hosted_rpc_lib/src/ctrl_api.c) and [ctrl_core.c](../../host/components/esp_hosted_rpc_lib/src/ctrl_core.c) which interact with platform
- The `esp_hosted_rpc_lib` works over `Host Serial Driver`
...
###### 3.1.2 Demo App
- Demo App is sample user application developed on top of `esp_hosted_rpc_lib` (hosted control library)
- This way, the end user can create the expected application with minimal or no change to the `esp_hosted_rpc_lib`
- Demo application creates control message using structures exposed in [ctrl_api.h](../../host/components/esp_hosted_rpc_lib/include/ctrl_api.h). These structures are used along with control path APIs to send a control request and parse a control response or a control event
