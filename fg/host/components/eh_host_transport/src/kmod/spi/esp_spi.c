...
#include "eh_common_tlv.h"  /* ESP_PRIV_*_VERSION, ESP_HOSTED_*_VERSION_* */
...
int process_init_event(u8 *evt_buf, u8 len)
{
	...
	/* Default to V1; upgraded if ESP_PRIV_HEADER_VERSION / ESP_PRIV_RPC_VERSION TLVs received */
	if (adapter) {
		adapter->hdr_ver_negotiated = ESP_HOSTED_HDR_VERSION_V1;
		adapter->rpc_ver_negotiated = ESP_HOSTED_RPC_VERSION_V1;
	}
	...
	while (len_left) {
		...
		/* Bounds guard: malformed TLV must not wrap len_left (u8). */
		if ((u16)tag_len + 2 > len_left) {
			esp_warn("TLV truncated: tag=0x%02x len=%u remaining=%u — abort\n",
			         *pos, tag_len, len_left);
			break;
		}

		esp_info("EVENT: %d\n", *pos);
		if (*pos == ESP_LINUX_FG_PRIV_CAPABILITY) {
			esp_warn("DEPRECATED: Legacy capability tag (0x%X) - please upgrade slave firmware to use MCU-style capabilities\n", *pos);
			adapter->capabilities = *(pos + 2);
			print_capabilities(*(pos + 2));
		} else if (*pos == ESP_LINUX_FG_PRIV_FIRMWARE_CHIP_ID){
			esp_warn("DEPRECATED: Legacy chip ID tag (0x%X) - please upgrade slave firmware\n", *pos);
			hardware_type = *(pos+2);
		} else if (*pos == ESP_LINUX_FG_PRIV_TEST_RAW_TP) {
			esp_warn("DEPRECATED: Legacy raw TP tag (0x%X) - please upgrade slave firmware\n", *pos);
			process_test_capabilities(*(pos + 2));
		} else if (*pos == ESP_LINUX_FG_PRIV_FW_DATA) {
			esp_warn("DEPRECATED: Legacy firmware data tag (0x%X) - please upgrade slave firmware\n", *pos);
			...
			esp_info("ESP chipset detected (MCU-style)\n");
			hardware_type = *(pos+2);
			...
			/* Informational — SDIO transport mode byte from MCU-style firmware */
			esp_info("SDIO transport mode: 0x%02X\n", *(pos + 2));
		} else if (*pos == ESP_PRIV_FEAT_CAPS) {
			/* feat_caps[8] array — 32 bytes, boot-time capability bitmask */
			if (tag_len == 32 && adapter) {
				u32 *fc = (u32 *)(pos + 2);
				int i;
				for (i = 0; i < 8; i++)
					adapter->feat_caps[i] = le32_to_cpu(fc[i]);
				esp_info("feat_caps: [0]=0x%08X [1]=0x%08X [2]=0x%08X [3]=0x%08X\n",
				         adapter->feat_caps[0], adapter->feat_caps[1],
				         adapter->feat_caps[2], adapter->feat_caps[3]);
			} else {
				esp_warn("feat_caps TLV unexpected len=%u (expected 32)\n", tag_len);
			}
		} else if (*pos == ESP_PRIV_HEADER_VERSION) {
			/* CP advertises its max wire-header version; agree on min(host, CP) */
			u8 cp_ver = *(pos + 2);
			u8 agreed = (cp_ver >= ESP_HOSTED_HDR_VERSION_V2)
				? ESP_HOSTED_HDR_VERSION_V2 : ESP_HOSTED_HDR_VERSION_V1;
			if (adapter)
				WRITE_ONCE(adapter->hdr_ver_negotiated, agreed);
			esp_info("Wire hdr ver: CP=%u agreed=%u\n", cp_ver, agreed);
		} else if (*pos == ESP_PRIV_RPC_VERSION) {
			/* CP advertises its max RPC protocol version; agree on min(host, CP) */
			u8 cp_rpc_ver = *(pos + 2);
			u8 agreed_rpc = (cp_rpc_ver >= ESP_HOSTED_RPC_VERSION_V2)
				? ESP_HOSTED_RPC_VERSION_V2 : ESP_HOSTED_RPC_VERSION_V1;
			if (adapter)
				WRITE_ONCE(adapter->rpc_ver_negotiated, agreed_rpc);
			esp_info("RPC ver: CP=%u agreed=%u\n", cp_rpc_ver, agreed_rpc);
		} else {
	...
	/*
	 * GAP-001 fix: send host→slave config frame including ACK TLVs
	 * (0x21 = ESP_PRIV_HEADER_VERSION_ACK, 0x23 = ESP_PRIV_RPC_VERSION_ACK).
	 * CP side parses these in host_to_slave_reconfig() to finalise negotiation.
	 */
	if (adapter) {
		u8 hdr_ack = READ_ONCE(adapter->hdr_ver_negotiated);
		u8 rpc_ack = READ_ONCE(adapter->rpc_ver_negotiated);
		ret = esp_send_host_config(adapter, hdr_ack, rpc_ack);
		if (ret)
			esp_warn("esp_send_host_config failed (%d) — CP stays at V1\n", ret);
	}
...
static int spi_dev_init(struct esp_spi_context *context)
{
	...
	if (!master) {
		...
		esp_info("** Please refer https://github.com/espressif/esp-hosted/blob/master/eh_fg/docs/Linux_based_host/porting_guide.md **\n");
		return -ENODEV;
	}
