...
int process_init_event(u8 *evt_buf, u8 len)
{
	...
	/*
	 * GAP-001 fix: send host→slave config frame with ACK TLVs.
	 * Shared helper defined in main.c; both SPI and SDIO call it here.
	 */
	{
		u8 hdr_ack = READ_ONCE(adapter->hdr_ver_negotiated);
		u8 rpc_ack = READ_ONCE(adapter->rpc_ver_negotiated);
		int cfg_ret = esp_send_host_config(adapter, hdr_ack, rpc_ack);
		if (cfg_ret)
			esp_warn("esp_send_host_config failed (%d)\n", cfg_ret);
	}
