...
#include "eh_host_transport_spi_hd.h"
#include "stats.h"
...
static int spi_hd_write_packet(interface_buffer_handle_t *buf_handle)
{
	...
	if (buf_handle->payload_len > MAX_SPI_HD_BUFFER_SIZE - eh_frame_hdr_size()) {
		ESP_LOGE(TAG, "Pkt len [%u] > Max [%u]. Drop",
				buf_handle->payload_len, MAX_SPI_HD_BUFFER_SIZE - eh_frame_hdr_size());
		result = ESP_FAIL;
		goto done;
	}
	...
	/* Build wire header via frame component (V1/V2, checksum handled internally) */
	{
		...
		h.payload_zcopy = buf_handle->payload_zcopy;
	...
	buf_needed = (len + eh_frame_hdr_size() + MAX_SPI_HD_BUFFER_SIZE - 1)
...
static esp_err_t spi_hd_push_pkt_to_queue(uint8_t * rxbuff, uint16_t len, uint16_t offset)
{
	...
	fres = eh_frame_decode(rxbuff, MAX_SPI_HD_BUFFER_SIZE, &buf_handle);
	if (fres == EH_FRAME_DUMMY)
		return ESP_OK;
	if (fres != EH_FRAME_OK) {
		ESP_LOGE(TAG, "SPI_HD frame_decode error %d, drop", fres);
		return ESP_FAIL;
	}
