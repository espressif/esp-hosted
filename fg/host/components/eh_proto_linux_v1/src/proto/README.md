...
> [!TIP]
> These both generated files should be already present in the repo and there is no need to build again for most cases.

## Manual re-generation of protobuf source files
...
2. Create python binding in `host/linux/user_space/python_demo_app/commands_map_py_to_c.py` and its python function in `host/linux/user_space/python_demo_app/commands_lib.py`.
...
User can test added functionality using `host/linux/user_space/python_demo_app/test.py`.
