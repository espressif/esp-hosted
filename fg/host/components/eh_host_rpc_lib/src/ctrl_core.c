...
static int ctrl_app_parse_event(CtrlMsg *ctrl_msg, ctrl_cmd_t *app_ntfy)
{
	...
	switch (ctrl_msg->msg_id) {
		case CTRL_EVENT_ESP_INIT: {
			...
			command_log("EVENT: DHCP/DNS status: iface[%d] link_up[%d] dhcp_up[%d] dns_up[%d] ip[%s] gw[%s] dns[%s]\n",
				app_ntfy->u.dhcp_dns_status.iface,
				app_ntfy->u.dhcp_dns_status.net_link_up,
				app_ntfy->u.dhcp_dns_status.dhcp_up,
				app_ntfy->u.dhcp_dns_status.dns_up,
				app_ntfy->u.dhcp_dns_status.dhcp_ip,
				app_ntfy->u.dhcp_dns_status.dhcp_gw,
				app_ntfy->u.dhcp_dns_status.dns_ip);
			break;
...
static int ctrl_app_parse_resp(CtrlMsg *ctrl_msg, ctrl_cmd_t *app_resp)
{
	...
	switch (ctrl_msg->msg_id) {
		case CTRL_RESP_BASE: {
			app_resp->resp_event_status = -ENOTSUP;
			goto fail_parse_ctrl_msg2;
			break;
		}
...
static int call_async_resp_callback(ctrl_cmd_t *app_resp)
{
	if (app_resp->msg_id == CTRL_RESP_BASE) {
			return CALLBACK_NOT_REGISTERED;
	} else if ((app_resp->msg_id < CTRL_RESP_BASE) ||
...
int ctrl_app_send_req(ctrl_cmd_t *app_req)
{
	...
	/* payload case is exact match to msg id in eh_config.pb-c.h */
