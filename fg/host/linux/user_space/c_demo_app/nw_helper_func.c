...
int up_softap_netdev(const network_info_t *info)
{
	...
	if (ret == SUCCESS) {
		printf("%s interface up, Please configure and run dnsmasq or some dhcp server on ethap0 interface\n", AP_INTERFACE);
		printf("Sample dnsmasq setup command: \n");
		printf("sudo ip addr add 192.168.4.1/24 dev ethap0 && \\\n");
		printf("sudo ip link set ethap0 up && \\\n");
		printf("sudo dnsmasq --no-daemon --interface=ethap0 --bind-interfaces \\\n");
		printf("  --dhcp-range=192.168.4.2,192.168.4.254,12h \\\n");
		printf("  --dhcp-option=3,192.168.4.1 --dhcp-option=6,192.168.4.1 \\\n");
		printf("  --log-queries --log-dhcp\n");
		printf("\n");
	} else {
		printf("Unable to up %s interface\n", AP_INTERFACE);
		goto close_sock;
