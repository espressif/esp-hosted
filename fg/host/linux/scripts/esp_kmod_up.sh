...
# Build and Load Kernel Module Script
#
...
check_dmesg_for_init_event() {
	if ! command -v dmesg &>/dev/null; then
		warn "'dmesg' command not found; skipping init event check."
		return
	fi

	log "Checking kernel log for process_init_event..."
	sleep 1
	if dmesg | tail -10 | grep -q "process_init_event"; then
		log "✅ process_init_event found in kernel log — init event reached host."
		log "You can now proceed to:"
		log "-----------------------"
		log "1. 🛠  Build the user space demo app"
		log "2. 🏃 Run the user space demo app."
		log ""
		log "-> For C demo, build using ./c_app_build.sh"
		log "-> For Python demo, build using ./py_app_build.sh"
	else
		warn "------------------------------------------------------------------------------------"
		warn "⚠️  process_init_event not found in kernel log (last 10 lines)."
		warn "What to do next?"
		warn "1. Check dmesg if esp event received (sometimes late, most likely not received)"
		warn "2. Please check your SDIO device tree config for correct SDIO config and clock to be <= 50MHz"
		warn "3. Check porting guide at https://github.com/espressif/esp-hosted/blob/master/esp_hosted_fg/docs/Linux_based_host/porting_guide.md"
		warn "------------------------------------------------------------------------------------"
	fi
}
...
log "--- Kernel module build and load finished ---"
log ""
log "Kernel module esp32_${IF_TYPE}.ko has been successfully built and loaded."
