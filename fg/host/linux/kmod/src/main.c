...
struct esp_adapter *g_esp_adapter = NULL;

/* Transport ops initialization */
extern int eh_host_transport_linux_ops_init(struct esp_adapter *adapter);
extern void eh_host_transport_linux_ops_deinit(void);
...
static int process_tx_packet (struct sk_buff *skb)
{
	...
	if (eh_host_transport_is_sleeping(&adapter)) {
		return NETDEV_TX_BUSY;
	}
	...
	if (!stop_data) {
		ret = eh_host_transport_send_skb(priv->adapter, skb);
...
static int esp_get_packets(struct esp_adapter *adapter)
{
	...
	skb = eh_host_transport_recv_skb(adapter);
...
/*
 * esp_send_host_config() — shared by SPI and SDIO kmod transports.
 *
 * Sends a host→slave PRIV_EVENT_INIT frame after the slave startup event
 * is processed.  Carries:
 *   SLAVE_CONFIG TLVs (0x44–0x48) — host capabilities + thresholds
 *   ESP_PRIV_HEADER_VERSION_ACK (0x21) — finalises wire-header version
 *   ESP_PRIV_RPC_VERSION_ACK    (0x23) — finalises RPC protocol version
 *
 * GAP-001 fix: without this, CP stays at V1 header / V1 RPC even if V2 was
 * negotiated, because it only upgrades after receiving the host ACK TLVs.
 */
int esp_send_host_config(struct esp_adapter *adapter, u8 hdr_ver_ack, u8 rpc_ver_ack)
{
#define HOST_CFG_PKT_SIZE 80  /* header(12) + event(2) + TLVs(7x3=21) + pad */
	struct sk_buff *skb;
	...
	skb = esp_alloc_skb(HOST_CFG_PKT_SIZE);
	if (!skb) {
		esp_err("esp_send_host_config: alloc_skb failed\n");
		return -ENOMEM;
	}

	memset(skb->data, 0, HOST_CFG_PKT_SIZE);
	...
	*pos++ = RCVD_ESP_FIRMWARE_CHIP_ID;        *pos++ = 1; *pos++ = 0; payload_len += 3;
	*pos++ = SLV_CONFIG_TEST_RAW_TP;           *pos++ = 1; *pos++ = 0; payload_len += 3;
	*pos++ = SLV_CONFIG_THROTTLE_HIGH_THRESHOLD; *pos++ = 1; *pos++ = 0; payload_len += 3;
	*pos++ = SLV_CONFIG_THROTTLE_LOW_THRESHOLD;  *pos++ = 1; *pos++ = 0; payload_len += 3;
	*pos++ = ESP_PRIV_HEADER_VERSION_ACK;      *pos++ = 1; *pos++ = hdr_ver_ack; payload_len += 3;
	*pos++ = ESP_PRIV_RPC_VERSION_ACK;         *pos++ = 1; *pos++ = rpc_ver_ack; payload_len += 3;

	event->event_len = (u8)payload_len;
	payload_len += 2;  /* + event_type(1) + event_len(1) */
	hdr->len = cpu_to_le16(payload_len);

	checksum = compute_checksum(skb->data,
	                            sizeof(struct esp_payload_header) + payload_len);
	hdr->checksum = cpu_to_le16(checksum);
	skb_trim(skb, sizeof(struct esp_payload_header) + payload_len);

	esp_info("host_config: hdr_ver_ack=%u rpc_ver_ack=%u\n", hdr_ver_ack, rpc_ver_ack);
	return esp_send_packet(adapter, skb);
#undef HOST_CFG_PKT_SIZE
}
EXPORT_SYMBOL(esp_send_host_config);
...
static int __init esp_init(void)
{
	...
	/* Initialize Linux transport ops and layer in single call */
	ret = eh_host_transport_linux_ops_init(adapter);
