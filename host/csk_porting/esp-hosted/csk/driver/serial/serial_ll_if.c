...
static int serial_ll_open(serial_ll_handle_t *serial_ll_hdl)
{
	...
	/* Queue - serial rx */
	int ret = csk_os_queue_create(&serial_ll_hdl->queue,
...
static int serial_ll_close(serial_ll_handle_t * serial_ll_hdl)
{
    ...
    if (serial_ll_hdl->queue) {
		csk_os_queue_delete(serial_ll_hdl->queue);
							 ...
							 uint16_t * rlen)
{
	...
	*rlen = 0 ;
	 ...
	 */
    if (csk_os_queue_receive(serial_ll_hdl->queue, &buf_handle, CSK_OS_WAIT_FOREVER) != 0) {
...
#define SERIAL_ALLOC_REALLOC_RDATA() \
	...
	serial_buf = (uint8_t *)hosted_malloc(r.len);
	if(!serial_buf) {
	...
	new_buf_handle.free_buf_handle = hosted_free;
	...
	if (0 != csk_os_queue_send(serial_ll_hdl->queue,
		    &new_buf_handle, CSK_OS_WAIT_FOREVER)) {
...
serial_ll_handle_t * serial_ll_init(void(*serial_rx_callback)(void))
{
	...
	if ((conn_num+1) < MAX_SERIAL_INTF) {
		...
		serial_ll_hdl = (serial_ll_handle_t *)hosted_malloc(sizeof(serial_ll_handle_t));
		if (! serial_ll_hdl) {
