...
int esp_serial_reinit(void *priv)
{
	...
	/* If a previous session left the class/chrdev registered but dropped the
	 * init flag, clean the stale state before trying to re-register.
	 */
	if (cl || dev_first) {
		esp_warn("stale serial state detected, forcing cleanup before reinit\n");
		if (cl) {
			class_destroy(cl);
			cl = NULL;
		}
