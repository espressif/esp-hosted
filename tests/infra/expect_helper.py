...
# Crash vocabulary lives in infra.signatures (one source). Name kept for the
# HW + emu test bodies that import FATAL_PATTERNS from here.
from infra.signatures import EXPECT_FATAL as FATAL_PATTERNS
...
def _expect_impl(dut, success, fail, timeout, use_exact):
    ...
    # EH_EXPECT_SCALE multiplies every expect budget (slow machines / CI).
    timeout = timeout * _env_float("EH_EXPECT_SCALE", 1.0)
    _want = success if isinstance(success, str) else str(success)
    _log.info("[expect] wait: %s  (<= %.0fs)",
              (_want[:70] + "…") if len(_want) > 70 else _want, timeout)
