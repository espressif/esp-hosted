...
def _find_repo_root(project_dir: str) -> str:
    ...
    CP and Host projects can live in different repos (this repo vs an external
    legacy mcu6 checkout), so the symlink target must be derived from the
    project's own location, not from where this file lives.
    """
...
def _ensure_component_symlink(project_dir: str):
    """Validate and create components/esp_hosted symlink pointing at the repo root.

    Every example in this repo builds against the in-tree esp_hosted — never a
    managed-component fetch from the registry. Examples that declare
    `espressif/esp_hosted` in idf_component.yml must already keep an override
    file alongside that pins it to local source; the symlink here makes the
    local source discoverable as `components/esp_hosted`.

    Symlink rule: must point INTO the same repo as the project. A cross-repo
    symlink is stale/buggy artifact from an earlier helper run and is removed.
    A real file/dir at that path is left alone.
    """
