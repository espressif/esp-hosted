...
def _flash_sequential(params, log):
    ...
    # CP flash
    if cp['need_cp_flash']:
        # Shared-UART boards (e.g. ESP32-P4-Function-EV) mux the CP's flash UART
        # through the host: the CP port is only reachable while the host sits
        # passive in download. Park the host in bootloader BEFORE the CP flash,
        # else the CP port is invisible / returns serial noise. Harmless on boards
        # with an independent CP UART (host just waits in download until its flash).
