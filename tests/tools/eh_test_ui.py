...
<header>
  ...
  <button class="primary" id="header-run-btn" onclick="quickRun()" style="margin-right:8px;" title="Switch to Test Execution and start a run">▶ Run Tests</button>
  ...
  <div id="tab-config" class="cfg-layout">
    ...
    <div class="cfg-content">
      ...
      <div class="card" id="sec-paths">
        <h3>Paths</h3>
        <div class="field"><label>Example source</label>
          <div class="radio-group">
            <label><input type="radio" name="paths_mode" value="this_repo" onchange="markDirty(); updatePathsModeUI()"> This repo (examples/&lt;feat&gt;/&lt;scen&gt;/mcu/)</label>
            <label><input type="radio" name="paths_mode" value="legacy" onchange="markDirty(); updatePathsModeUI()"> Legacy (external mcu6 host_base)</label>
          </div>
        </div>
        <div class="field"><label>IDF Path</label><input type="text" id="idf_path" onchange="markDirty()"></div>

        <!-- Legacy-only fields: hidden in this_repo mode -->
        <div id="legacy-paths-block">
          <div class="field"><label>Host Base</label>
            <input type="text" id="host_base" onchange="markDirty()" placeholder="auto (= &lt;repo&gt;/_upstream_mcu/examples symlink)"></div>
          <div class="field"><label>CP Base (kconfig parser only)</label>
            <input type="text" id="cp_base" onchange="markDirty()" placeholder="external FG repo cp_mcu path"></div>
        </div>

        <!-- this_repo-only field: this_repo_root override (auto by default) -->
        <div id="this-repo-paths-block">
          <div class="field"><label>This-repo root</label>
            <input type="text" id="this_repo_root" onchange="markDirty()" placeholder="auto (= containing repo of tests)"></div>
        </div>
...
function populateForm() {
  ...
  checkRadio('paths_mode', paths.mode || 'this_repo');
  updatePathsModeUI();   // toggle legacy vs this_repo blocks
  document.getElementById('idf_path').value = paths.idf_path || '~/esp-idf';
  document.getElementById('this_repo_root').value = paths.this_repo_root || 'auto';
...
function gatherForm() {
  ...
  return {
    ...
    paths: {
      mode: (document.querySelector('input[name="paths_mode"]:checked') || {}).value || 'this_repo',
      this_repo_root: document.getElementById('this_repo_root').value || 'auto',
...
// Show/hide legacy vs this_repo path fields based on the selected mode.
function updatePathsModeUI() {
  const m = (document.querySelector('input[name="paths_mode"]:checked') || {}).value || 'this_repo';
  document.getElementById('legacy-paths-block').style.display    = (m === 'legacy')    ? '' : 'none';
  document.getElementById('this-repo-paths-block').style.display = (m === 'this_repo') ? '' : 'none';
}

// Header CTA: switch to Tests tab and start a run with current config.
// Saves any unsaved config first so the runner sees the latest values.
async function quickRun() {
  if (dirty) {
    await saveConfig();
  }
  switchTab('tests');
  await new Promise(r => setTimeout(r, 50));   // let the tab render
  startRun(false);
}
