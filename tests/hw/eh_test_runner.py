...
def optimize_order(pairs, cfg):
    ...
    while remaining:
        def score(item):
            _, pcfg = item
            same_cp = cfg.resolve_cp_path(pcfg) == cur_cp
            same_host = cfg.resolve_host_path(pcfg) == cur_host
        ...
        cur_cp = cfg.resolve_cp_path(best_cfg)
        cur_host = cfg.resolve_host_path(best_cfg)
...
def main():
    ...
    parser.add_argument('--src', choices=['this_repo', 'legacy'],
                        help='Example source: this_repo=examples/<feat>/<scen>/mcu/{cp,host}/, '
                             'legacy=external mcu6 host_base + this-repo cp. '
                             'Overrides paths.mode in env.json (default this_repo).')
    ...
    # CLI --src overrides env.json paths.mode via env var (config.py reads it).
    if args.src:
        os.environ['EH_TEST_MODE'] = args.src
    ...
    if order == 'auto' and len(pairs) > 1:
        pairs = optimize_order(pairs, cfg)
        print(f'  Order: auto (optimized for {len(pairs)} suites)')
    elif order == 'alphabetical':
        pairs = dict(sorted(pairs.items()))

    print(f'  Path resolution: {cfg.describe_paths()}')
    ...
    if args.dry_run:
        ...
        for name, pcfg in pairs.items():
            print(f'  {name}: build {cfg.resolve_cp_path(pcfg)} + {cfg.resolve_host_path(pcfg)}, flash, run {pcfg["tests"]}')
        return 0
    ...
    for pair_name, pair_cfg in pairs.items():
        # Per-mode parity gate. 'ok' = run; 'incomplete'/'missing' = skip with reason.
        cov = pair_cfg.get('coverage', {})
        cov_status = cov.get(cfg.path_mode, 'ok')
        if cov_status != 'ok':
            print(f'  SKIP {pair_name}: coverage[{cfg.path_mode}]={cov_status} '
                  f'— {pair_cfg.get("description","")}')
            continue
        cp_path = cfg.resolve_cp_path(pair_cfg)
        host_path = cfg.resolve_host_path(pair_cfg)
        if not cp_path or not Path(cp_path).is_dir():
            print(f'  SKIP {pair_name}: cp path missing ({cp_path})')
            continue
        if not host_path or not Path(host_path).is_dir():
            print(f'  SKIP {pair_name}: host path missing ({host_path})')
            continue
