...
#error "Invalid UART Port Selected"
	...
	EH_CHECK_OK(uart_param_config(EH_CP_BT_UART, &uart_config));

	gdma_channel_alloc_config_t tx_channel_config = {
		.flags.reserve_sibling = 1,
		.direction = GDMA_CHANNEL_DIRECTION_TX,
	};
	EH_CHECK_OK(gdma_new_channel(&tx_channel_config, &s_tx_channel));

	gdma_channel_alloc_config_t rx_channel_config = {
		.direction = GDMA_CHANNEL_DIRECTION_RX,
		.sibling_chan = s_tx_channel,
	};
	EH_CHECK_OK(gdma_new_channel(&rx_channel_config, &s_rx_channel));
	...
	s_uhci_hw->escape_conf.val = 0;  /* disable sw flow control */
