...
void process_hci_rx_pkt(uint8_t *payload, uint16_t payload_len)
{
	...
	/* VHCI needs the H4 byte ahead of the body; it sits in the wire
	 * header's pkt_type slot, immediately before payload[0]. */
	payload--;
	payload_len++;
...
esp_err_t eh_cp_bt_init(void)
{
	...
	if (esp_bt_controller_get_status() >= ESP_BT_CONTROLLER_STATUS_INITED) {
		ESP_LOGD(TAG, "controller already inited; no-op");
		return ESP_OK;
	}
	...
	EH_CHECK_OK(esp_read_mac(mac, ESP_MAC_BT));
...
esp_err_t eh_cp_bt_deinit(bool mem_release)
{
	...
	if (esp_bt_controller_get_status() == ESP_BT_CONTROLLER_STATUS_IDLE) {
		ESP_LOGD(TAG, "controller already deinited; no-op");
		return ESP_OK;
	}
...
esp_err_t eh_cp_bt_enable(void)
{
	...
	if (esp_bt_controller_get_status() >= ESP_BT_CONTROLLER_STATUS_ENABLED) {
		ESP_LOGD(TAG, "controller already enabled; no-op");
		return ESP_OK;
	}
...
#if EH_CP_FEAT_BT_READY
	if (esp_bt_controller_get_status() < ESP_BT_CONTROLLER_STATUS_ENABLED) {
		ESP_LOGD(TAG, "controller already disabled; no-op");
		return ESP_OK;
	}
