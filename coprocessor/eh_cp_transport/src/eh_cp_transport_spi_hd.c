...
#define EH_CP_DATAREADY_ACTIVE_HIGH    CONFIG_EH_TRANSPORT_CP_SPI_HD_DR_ACTIVE_HIGH
...
#define GPIO_MASK_DATA_READY        (1ULL << GPIO_DATA_READY)
  ...
  #define EH_CP_DR_VAL_ACTIVE           GPIO_OUT_W1TS_REG
  #define EH_CP_DR_VAL_INACTIVE         GPIO_OUT_W1TC_REG
  #define EH_CP_DR_PULL_REGISTER        GPIO_PULLDOWN_ONLY
#else
  #define EH_CP_DR_VAL_ACTIVE           GPIO_OUT_W1TC_REG
  #define EH_CP_DR_VAL_INACTIVE         GPIO_OUT_W1TS_REG
...
static void spi_hd_rx_task(void* pvParameters)
{
	...
	// wait for data path to be opened by host before servicing rx
...
static interface_handle_t * esp_spi_hd_init(void)
{
	 ...
	 */
	gpio_set_pull_mode(GPIO_DATA_READY, EH_CP_DR_PULL_REGISTER);
	gpio_set_pull_mode(GPIO_SCLK, GPIO_PULLUP_ONLY);
	gpio_set_pull_mode(GPIO_CS, GPIO_PULLUP_ONLY);

	show_config();
#if !EH_CP_DATAREADY_ACTIVE_HIGH
	ESP_LOGI(TAG, "transport[cp]: SPI_HD DataReady: Active Low");
#else
	ESP_LOGI(TAG, "transport[cp]: SPI_HD DataReady: Active High");
#endif
	...
	// Host data-path wait happens in spi_hd_rx_task, not here, so init()
	// returns promptly and core setup (feature auto-init) runs at boot.
                            ...
                            const uint32_t feat_caps[8])
{
	...
	{
		eh_frame_cfg_t frame_cfg = EH_FRAME_CFG_CP_LINUX_802_3_SPI_HD_DEFAULT;
...
#if EH_TLV_V1
	{
		...
		if (rc) { ESP_LOGE(TAG, "TLV v1 overflow"); spi_hd_buffer_tx_free(buf_handle.payload); xSemaphoreGive(mempool_tx_sem); return; }
	}
#endif
