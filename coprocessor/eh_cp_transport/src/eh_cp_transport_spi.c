...
/* Transport config banner — mirrors upstream MCU show_configuration(). */
static void show_config(void)
{
	ESP_LOGI(TAG, "transport[cp]: SPI ctrl=%d mode=%d "
		"MOSI=%d MISO=%d CLK=%d CS=%d HANDSHAKE=%d DATA_READY=%d",
		(int)EH_CP_SPI_CONTROLLER, (int)EH_CP_SPI_MODE,
		(int)EH_CP_SPI_PIN_MOSI, (int)EH_CP_SPI_PIN_MISO,
		(int)EH_CP_SPI_PIN_CLK,  (int)EH_CP_SPI_PIN_CS,
		(int)EH_CP_SPI_PIN_HANDSHAKE,
		(int)EH_CP_SPI_PIN_DATA_READY);
}
...
/* De-assert HS on CS rather than at end of transaction. */
...
#define EH_CP_HANDSHAKE_ACTIVE_HIGH    1
#define EH_CP_DATAREADY_ACTIVE_HIGH    1
...
#define EH_CP_CS_INTR_TO_CLEAR_HS                        GPIO_INTR_ANYEDGE
#else
#define EH_CP_CS_INTR_TO_CLEAR_HS                        GPIO_INTR_NEGEDGE
#endif

#if EH_CP_HANDSHAKE_ACTIVE_HIGH
  #define EH_CP_HS_VAL_ACTIVE                            (1)
  #define EH_CP_HS_VAL_INACTIVE                          (0)
  #define EH_CP_HS_PULL_REGISTER                         GPIO_PULLDOWN_ONLY
#else
  #define EH_CP_HS_VAL_ACTIVE                            (0)
  #define EH_CP_HS_VAL_INACTIVE                          (1)
  #define EH_CP_HS_PULL_REGISTER                         GPIO_PULLUP_ONLY
#endif

#if EH_CP_DATAREADY_ACTIVE_HIGH
  #define EH_CP_DR_VAL_ACTIVE                            (1)
  #define EH_CP_DR_VAL_INACTIVE                          (0)
  #define EH_CP_DR_PULL_REGISTER                         GPIO_PULLDOWN_ONLY
#else
  #define EH_CP_DR_VAL_ACTIVE                            (0)
  #define EH_CP_DR_VAL_INACTIVE                          (1)
  #define EH_CP_DR_PULL_REGISTER                         GPIO_PULLUP_ONLY
#endif

/* Max SPI slave CLK (IO_MUX): ESP32 10MHz; C2/C3/S2/S3 40MHz; C6 26MHz. */
...
static inline void spi_mempool_create()
{
	...
	/* Reuse TX mempool for RX (same size). */
...
#define set_handshake_gpio()     ESP_EARLY_LOGD(TAG, "+ set handshake gpio");gpio_set_level(GPIO_HANDSHAKE, EH_CP_HS_VAL_ACTIVE);
#define reset_handshake_gpio()   ESP_EARLY_LOGD(TAG, "- reset handshake gpio");gpio_set_level(GPIO_HANDSHAKE, EH_CP_HS_VAL_INACTIVE);

volatile uint8_t data_ready_flag = EH_CP_HS_VAL_INACTIVE;
...
#ifdef CONFIG_ESP_ENABLE_RX_PRIORITY_QUEUES
	load_percent = (queue_load*100/SPI_RX_WIFI_QUEUE_SIZE);
#else
...
/* Emits Linux FG + MCU + V2 TLVs together for universal host compatibility. */
                            ...
                            const uint32_t feat_caps[8])
{
...
#if EH_TLV_V1
	{
		...
		if (rc) { ESP_LOGE(TAG, "TLV v1 overflow"); return; }
	}
#endif
...
#if EH_TLV_V2
	{
		...
		rc = eh_tlv_pack_v2(&tlv,
			CONFIG_IDF_FIRMWARE_CHIP_ID, cap, raw_tp_cap,
			ext_cap, fw_version,
			SPI_RX_TOTAL_QUEUE_SIZE, SPI_TX_TOTAL_QUEUE_SIZE,
...
static uint8_t * get_next_tx_buffer(uint32_t *len)
{
	...
	/* No real data; clear DR line to signal idle. */
...
static int process_spi_rx(interface_buffer_handle_t *buf_handle)
{
	...
	raw_len = (uint16_t)SPI_BUFFER_SIZE;
...
static void spi_transaction_post_process_task(void* pvParameters)
{
	...
	for (;;) {
...
#if HS_DEASSERT_ON_CS
		/* Some MCUs delay deasserting CS; wait to avoid losing data. */
		...
		if (likely(spi_trans->rx_buffer)) {
			...
			/* On error, free rx_buffer (success path frees later). */
...
static void register_hs_disable_pin(uint32_t gpio_num)
{
	if (gpio_num != -1) {
		...
		gpio_set_intr_type(gpio_num, EH_CP_CS_INTR_TO_CLEAR_HS);
...
static interface_handle_t * esp_spi_init(void)
{
	...
	if (!hosted_constructs_init_done) {
...
#if USE_STATIC_DUMMY_BUFFER
		{
			/* Boot V1; PRIV handshake may re-init to V2. */
			eh_frame_cfg_t frame_cfg = EH_FRAME_CFG_CP_LINUX_802_3_SPI_DEFAULT;
		...
		/* Pull-ups to avoid rogue pulses when no master connected. */
		gpio_set_pull_mode(CONFIG_EH_TRANSPORT_CP_SPI_GPIO_HANDSHAKE, EH_CP_HS_PULL_REGISTER);
		gpio_set_pull_mode(CONFIG_EH_TRANSPORT_CP_SPI_GPIO_DATA_READY, EH_CP_DR_PULL_REGISTER);
...
static int32_t esp_spi_write(interface_handle_t *handle, interface_buffer_handle_t *buf_handle)
{
	...
	xSemaphoreGive(spi_tx_sem);
#else
	/* Enqueue BEFORE asserting DR so the post-process task can't observe an
	 * empty queue, clear DR and send a dummy while the real packet waits. */
