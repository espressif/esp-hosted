 ...
 *
 * ESP-Hosted MCU host — Memory Monitor Example
 *
 * Ported 1:1 from upstream esp_hosted_mcu6/examples/host_hosted_cp_meminfo.
 * Host configures the coprocessor mem-monitor over RPC and logs every
 * mem-info event the CP sends back.
 */
...
/* Delay by twice the monitor interval to give time for a mem-monitor
 * event to be sent before / after Wi-Fi state changes. */
 ...
 *
 * Note: external threshold values trigger the event only if external
 * memory (SPIRAM) is enabled on the co-processor.
 */
...
/**
 * Handler for ESP-Hosted events.
 * See station_example.c for the Wi-Fi and IP event handler.
 */
                                     ...
                                     int32_t event_id, void* event_data)
{
    ...
    if (event_id == ESP_HOSTED_EVENT_MEM_MONITOR) {
        ...
        ESP_LOGI(TAG, "Got UNKNOWN ESP_HOSTED event %"PRId32, event_id);
    }
}
...
static void app_init_once(void)
{
    ...
    /* Binary semaphore the app waits on until ESP-Hosted transport is up. */
...
static esp_err_t app_mem_monitor_set(esp_hosted_curr_mem_info_t *curr_mem_info)
{
    ...
    esp_hosted_config_mem_monitor_t config = {
        ...
        /* Don't change monitor configuration for a one-time query. */
...
static void print_mem_info_resp(esp_hosted_curr_mem_info_t *info)
{
