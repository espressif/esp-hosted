...
Before project configuration and build, be sure to set the correct chip target using `eh.py set-target <chip_name>`.
...
Open the project configuration menu (`eh.py menuconfig`).
...
Run `eh.py -p PORT flash monitor` to build, flash and monitor the project.
...
This example can run on ESP Chips without Wi-Fi using ESP-Hosted. See the [upper level `README.md`](../README.md) for information on the two-chip solution.
