...
#error "Disable extconn as per esp-hosted-mcu doc and try again"
    ...
    /* Bring up hosted transport before any wifi_cmd_* call, otherwise
     * wifi_cmd init can hit "request: rpc not ready" and abort. */
    ret = esp_netif_init();
    ...
    /* Register commands. Host CLI (host_power_save and friends) is
     * registered by eh_host_feat_cli's auto-init during eh_host_init —
