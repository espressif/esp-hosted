...
void app_main(void)
{
	...
	/* nw_split's default netif mode is APP_EXTERNAL: the app must create
	 * WIFI_STA_DEF before esp_hosted_init() so the feature's auto-init can
	 * find it. Without this the split path aborts (ESP_ERR_INVALID_STATE). */
	esp_netif_create_default_wifi_sta();

	ESP_ERROR_CHECK(esp_hosted_init());
	esp_hosted_connect_to_slave();

	/* nw_split feature is auto-initialised on the CP side by
	 * EH_CP_EXT_REGISTER inside the eh_cp_feat_nw_split component;
	 * the host just establishes the transport via the calls above. */
