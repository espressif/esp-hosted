...
static esp_mqtt_client_handle_t example_mqtt_init_internal(void)
{
	...
	struct hostent *host_entry = gethostbyname("mqtt.eclipseprojects.io");
	if (host_entry == NULL) {
		ESP_LOGE(TAG, "DNS resolution failed for mqtt.eclipseprojects.io");
	} else {
		ESP_LOGI(TAG, "DNS resolved mqtt.eclipseprojects.io to: %s", 
			inet_ntoa(*((struct in_addr*)host_entry->h_addr_list[0])));
	}

	esp_mqtt_client_config_t mqtt_cfg = {
		.broker.address.uri = CONFIG_BROKER_URL,
		//.broker.address.uri = "mqtt://broker.hivemq.com:1883" //alternative broker, comment above line
	};
