 ...
 */
/*
 * esp_hosted_bt_stack.h — one explicit entry point to bind a BT host stack to
 ...
 *   esp_hosted_bt_stack_setup(&cfg);         // controller (+MAC) up + HCI bound
 *   // then the stack's own lifecycle (app-owned): esp_bluedroid_init/enable
 *   // or nimble_port_init().
 */
#ifndef ESP_HOSTED_BT_STACK_H_
#define ESP_HOSTED_BT_STACK_H_
...
typedef enum {
    ESP_HOSTED_BT_STACK_NIMBLE,
    ESP_HOSTED_BT_STACK_BLUEDROID,
    ESP_HOSTED_BT_STACK_CUSTOM,
} esp_hosted_bt_stack_t;

typedef struct {
    esp_hosted_bt_stack_t stack;                    /* which host stack to bind */
...
#  define ESP_HOSTED_BT_STACK_DEFAULT_TYPE ESP_HOSTED_BT_STACK_NIMBLE
#elif defined(CONFIG_BT_BLUEDROID_ENABLED)
#  define ESP_HOSTED_BT_STACK_DEFAULT_TYPE ESP_HOSTED_BT_STACK_BLUEDROID
#else
#  define ESP_HOSTED_BT_STACK_DEFAULT_TYPE ESP_HOSTED_BT_STACK_CUSTOM
#endif

#define ESP_HOSTED_BT_STACK_CONFIG_DEFAULT() (esp_hosted_bt_stack_cfg_t){ \
    .stack = ESP_HOSTED_BT_STACK_DEFAULT_TYPE, \
...
#define ESP_HOSTED_BT_STACK_CONFIG_NIMBLE() (esp_hosted_bt_stack_cfg_t){ \
    .stack = ESP_HOSTED_BT_STACK_NIMBLE, .bring_up_controller = true }
#define ESP_HOSTED_BT_STACK_CONFIG_BLUEDROID() (esp_hosted_bt_stack_cfg_t){ \
    .stack = ESP_HOSTED_BT_STACK_BLUEDROID, .bring_up_controller = true }
#define ESP_HOSTED_BT_STACK_CONFIG_CUSTOM(rx_fn, rx_ctx) (esp_hosted_bt_stack_cfg_t){ \
    .stack = ESP_HOSTED_BT_STACK_CUSTOM, .bring_up_controller = true, \
