# bleprph_gatt (`bluetooth/host_mcu/nimble/examples/hosted_hci/bleprph_gatt`)

Connectable BLE peripheral exposing one custom 128-bit GATT service with two
characteristics (read/write + notify). NimBLE counterpart to
`../../bluedroid/ble_gatt_server/`. Hosted brings up the remote BLE
controller via `esp_hosted_hci_nimble_setup()` immediately before
`nimble_port_init()`; everything after is verbatim NimBLE.
...
```bash
cd examples/bluetooth/host_mcu/nimble/examples/hosted_hci/bleprph_gatt
...
- Test with any generic BLE explorer — LightBlue, nRF Connect,
  `bluetoothctl`.

- Notifications are pushed via `ble_gatts_notify_custom` on the
  notify characteristic at the configured cadence.

- `esp_hosted_hci_nimble_setup()` is the only hosted-specific addition
  on top of upstream NimBLE init.
