# BLE Advertise Minimal (`bluetooth/host_mcu/bluedroid/examples/hosted_hci/ble_advertise_minimal`)

Smallest possible Bluedroid + ESP-Hosted example: brings the BT controller up
on the co-processor, attaches Bluedroid's HCI driver via the shared
`esp_hosted_hci_bluedroid` bridge, then starts non-connectable BLE
advertising. Teaching companion to the larger Bluedroid samples; steps 1-3
are the only ESP-Hosted-specific code, everything after `esp_bluedroid_init()`
is standard IDF Bluedroid.
...
```bash
cd examples/bluetooth/host_mcu/bluedroid/examples/hosted_hci/ble_advertise_minimal
...
- ESP32 CP supplies BLE 4.2 only — do **not** enable
  `CONFIG_BT_BLE_50_FEATURES_SUPPORTED=y` against an ESP32 CP, the build
  will fail.

- For a connectable peripheral with GATT services, see `ble_gatt_server`.
