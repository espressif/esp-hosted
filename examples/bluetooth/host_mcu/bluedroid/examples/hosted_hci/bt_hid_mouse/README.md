# BT Classic HID Mouse (`bluetooth/host_mcu/bluedroid/examples/hosted_hci/bt_hid_mouse`)

Acts as a Bluetooth Classic HID mouse — pairs with a host (PC or phone)
and sends mouse-movement reports. Ports upstream
`host_bluedroid_bt_hid_mouse_device` verbatim; the only ESP-Hosted-specific
change is the HCI driver, supplied by the shared `esp_hosted_hci_bluedroid`
bridge (`../../../components/`). Bluedroid + HID device profile run on the
application MCU; the BR/EDR controller runs on the ESP-Hosted co-processor.
...
```bash
cd examples/bluetooth/host_mcu/bluedroid/examples/hosted_hci/bt_hid_mouse
...
- This is a **Classic Bluetooth** profile (`CONFIG_BT_CLASSIC_ENABLED=y`,
  `CONFIG_BT_HID_DEVICE_ENABLED=y`). C/S/H-series CPs are BLE-only and
  will fail at controller init — use an ESP32 CP.
