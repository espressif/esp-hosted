# Classic BT Discovery (`bluetooth/host_mcu/bluedroid/examples/hosted_hci/classic_bt_discovery`)

Performs a Classic Bluetooth (BR/EDR) device inquiry and prints every
device the controller sees during the discovery window. Uses
`esp_bt_gap_*` (Classic) rather than `esp_ble_gap_*`; the bridge layer
is identical to the BLE examples — Classic vs BLE is purely a host-stack
concern. Bluedroid runs on the application MCU; the BR/EDR controller
runs on the ESP-Hosted co-processor.
...
The co-processor is the BR/EDR (Classic Bluetooth) controller — Classic BT is supported only on an ESP32 CP. The example's `sdkconfig.defaults` already enables the Bluetooth controller-only profile with BT HCI carried over the host bus (VHCI) — you only need to select the transport (must match the host):
...
`CONFIG_ESP_HOSTED_CP_FEAT_BT` requires `CONFIG_ESP_HOSTED_CP_BT_ENABLED` (the Kconfig warns otherwise); both are set above. Classic BT (BR/EDR) additionally requires an ESP32 CP (`eh.py set-target esp32`) - the controller-only profile above provides BR/EDR only on ESP32.
...
```bash
cd examples/bluetooth/host_mcu/bluedroid/examples/hosted_hci/classic_bt_discovery
...
- C/S/H-series CPs are BLE-only and will fail at controller init — use
  an ESP32 CP.

- The host enables `CONFIG_BTDM_CTRL_MODE_BR_EDR_ONLY=y` to drive
  Bluedroid's BR/EDR feature gates even though the local controller is
  disabled (CP supplies the controller).
