# BLE GATT Server (`bluetooth/host_mcu/bluedroid/examples/hosted_hci/ble_gatt_server`)

Connectable BLE peripheral with a single custom 128-bit GATT service
exposing a read/write characteristic and a notify characteristic. Standard
Bluedroid GATT-server pattern (profile registration, GAP + GATTS callbacks,
attribute table); Bluedroid runs on the application MCU and the BT
controller runs on the ESP-Hosted co-processor via the shared
`esp_hosted_hci_bluedroid` bridge. Pair with LightBlue, nRF Connect, or
`bluetoothctl`.
...
```bash
cd examples/bluetooth/host_mcu/bluedroid/examples/hosted_hci/ble_gatt_server
...
- BLE 4.2 by default — works against any BT-capable CP.
- For BLE 5.0 features (extended advertising, 2M PHY), set
  `CONFIG_BT_BLE_50_FEATURES_SUPPORTED=y` **and** use a C/H-series CP
  (ESP32 CP is BLE 4.2 only).

- Test apps: LightBlue Explorer (iOS/macOS), nRF Connect (Android/iOS),
