...
static eh_host_port_mutex_t *s_list_lock = NULL;
...
{
    if (s_list_lock == NULL) {
        s_list_lock = eh_host_port_mutex_create();
        if (s_list_lock == NULL) {
            return ESP_ERR_NO_MEM;
        }
    }
    eh_host_port_mutex_lock(s_list_lock);
    return ESP_OK;
}
...
{
    assert(s_list_lock);
    eh_host_port_mutex_unlock(s_list_lock);
    if (s_esp_netif_counter == 0) {
        eh_host_port_mutex_destroy(s_list_lock);
        s_list_lock = NULL;
    }
}
