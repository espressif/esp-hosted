...
The core packet routing logic resides in `nw_split_router.c` and `nw_split_router.h` on the slave side, managing all packet routing decisions. The packet routing logic implementation:

https://github.com/espressif/esp-hosted/blob/ba39e6392f4f1c2e9ace3c5fa463bab34a42cb31/esp_hosted_fg/esp/esp_driver/network_adapter/main/nw_split_router.c#L347-L520
...
All incoming packets are evaluated by `nw_split_router.c`. The slave routes packets based on these rules:
...
**Common Issues:**
  ...
  * **DHCP not completing:**
      * Verify where the DHCP packet is destined by checking the `nw_split_router.c` logic related to DHCP (refer to lines [38-42 here](https://github.com/espressif/esp-hosted/blob/ba39e6392f4f1c2e9ace3c5fa463bab34a42cb31/esp_hosted_fg/esp/esp_driver/network_adapter/main/nw_split_router.c#L38-L42)).
  * **Same port needing to be used by both slave and host:**
      * This is already supported for iperf on port 5001. You can amend the packet routing logic in `nw_split_router.c` to handle other specific ports similarly if needed.
