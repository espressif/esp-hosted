...
{
	g_h.funcs->_h_post_semaphore(read_sem);
}
...
{
	int s = g_h.funcs->_h_thread_cancel(ctrl_rx_thread_handle);
	if (s != 0) {
...
static ctrl_cmd_t * get_response(int *read_len, int timeout_sec)
{
	...
	/* 3. Wait for response */
	ret = g_h.funcs->_h_get_semaphore(read_sem, timeout_sec);
	if (ret) {
		if (errno == ETIMEDOUT)
...
int ctrl_app_send_req(ctrl_cmd_t *app_req)
{
	...
	ret = g_h.funcs->_h_get_semaphore(ctrl_req_sem, WAIT_TIME_B2B_CTRL_REQ);
	if (ret) {
...
int deinit_hosted_control_lib_internal(void)
{
	...
	if (read_sem && g_h.funcs->_h_destroy_semaphore(read_sem)) {
		ret = FAILURE;
...
int init_hosted_control_lib_internal(void)
{
	...
	/* semaphore init */
	read_sem = g_h.funcs->_h_create_binary_semaphore();
	...
	g_h.funcs->_h_get_semaphore(read_sem, HOSTED_SEM_BLOCKING);
