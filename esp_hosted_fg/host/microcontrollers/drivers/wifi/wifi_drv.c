...
semaphore_handle_t transport_active_sem;
...
{
    transport_active_sem = g_h.funcs->_h_create_binary_semaphore();
	if (!transport_active_sem)
...
{
	g_h.funcs->_h_get_semaphore(transport_active_sem, portMAX_DELAY); //Wait until slave is ready
