...
static stm_ret_t sdio_rx_esp32(void)
{
	...
	/* Allocate rx buffer */
	rxbuff = (uint8_t *)g_h.funcs->_h_malloc(MAX_SDIO_BUFFER_SIZE);
	assert(rxbuff);
	g_h.funcs->_h_memset(rxbuff, 0, MAX_SDIO_BUFFER_SIZE);
...
static void process_rx_task(void const* pvParameters)
{
	...
	while (1) {
		if (hosted_dequeue_item(from_slave_queue, &buf_handle, HOSTED_BLOCK_MAX))
			continue;
		...
		if (buf_handle.if_type == ESP_SERIAL_IF) {
			...
			g_h.funcs->_h_memcpy(serial_buf, payload, buf_handle.payload_len);
...
static void tx_task(void const* pvParameters)
{
	...
	for (;;) {
		...
		if (0 == hosted_dequeue_item(to_slave_queue, &buf_handle, HOSTED_BLOCK_MAX)) {
			...
			/* Copy payload */
			g_h.funcs->_h_memcpy(payload, buf_handle.payload, buf_handle.payload_len);
			/* Send packet */
			xSemaphoreTake(transmit_mux, HOSTED_BLOCK_MAX);
...
void transport_init(void(*transport_evt_handler_fp)(uint8_t))
{
	...
	/* Queue - tx */
	to_slave_queue = hosted_create_queue(TO_SLAVE_QUEUE_SIZE,
			sizeof(interface_buffer_handle_t));
	assert(to_slave_queue);

	/* Queue - rx */
	from_slave_queue = hosted_create_queue(FROM_SLAVE_QUEUE_SIZE,
			sizeof(interface_buffer_handle_t));
