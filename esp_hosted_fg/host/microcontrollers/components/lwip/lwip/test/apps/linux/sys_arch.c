...
/* pthread key to *our* thread local storage */
...
introduce_thread(pthread_t id)
{
  ...
  thread = (struct sys_thread *)mem_malloc(sizeof(struct sys_thread));
...
sys_thread_new(const char *name, lwip_thread_fn function, void *arg, int stacksize, int prio)
{
  ...
  thread_data = (struct thread_wrapper_data *)mem_malloc(sizeof(struct thread_wrapper_data));
  thread_data->arg = arg;
