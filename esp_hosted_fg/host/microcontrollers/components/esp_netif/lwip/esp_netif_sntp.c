...
{
    esp_err_t ret = ESP_OK;
    s_storage = g_h.funcs->_h_calloc(1, sizeof(sntp_storage_t) + // allocate space for servers only if we are supposed to refresh the settings
    ...
    if (s_storage) {
        ...
        if (storage->num_of_servers) {
            for (int i = 0; i < storage->num_of_servers; ++i) {
                g_h.funcs->_h_free(storage->servers[i]);
            }
