...
esp_err_t esp_netif_init(void)
{
    if (!sys_thread_tcpip(LWIP_CORE_IS_TCPIP_INITIALIZED)) {
        ...
        extern void lwip_init_tcp_isn(u32_t boot_time, const u8_t *secret_16_bytes);
...
esp_netif_t *esp_netif_new(const esp_netif_config_t *esp_netif_config)
{
    ...
    esp_netif_t *esp_netif = g_h.funcs->_h_calloc(1, sizeof(struct esp_netif_obj));
    if (!esp_netif) {
    ...
    // Create ip info
    esp_netif_ip_info_t *ip_info = g_h.funcs->_h_calloc(1, sizeof(esp_netif_ip_info_t));
    if (!ip_info) {
        ESP_LOGE(TAG, "Failed to allocate %d bytes (free heap size %d)", sizeof(esp_netif_ip_info_t),
                 esp_get_free_heap_size());
        g_h.funcs->_h_free(esp_netif);
        return NULL;
    }
    esp_netif->ip_info = ip_info;

    // creating another ip info (to store old ip)
    ip_info = g_h.funcs->_h_calloc(1, sizeof(esp_netif_ip_info_t));
    if (!ip_info) {
        ESP_LOGE(TAG, "Failed to allocate %d bytes (free heap size %d)", sizeof(esp_netif_ip_info_t),
                 esp_get_free_heap_size());
        g_h.funcs->_h_free(esp_netif->ip_info);
        g_h.funcs->_h_free(esp_netif);
        return NULL;
    }
    ...
    struct netif * lwip_netif = g_h.funcs->_h_calloc(1, sizeof(struct netif));
    if (!lwip_netif) {
        ESP_LOGE(TAG, "Failed to allocate %d bytes (free heap size %d)", sizeof(struct netif),
                 esp_get_free_heap_size());
        g_h.funcs->_h_free(esp_netif->ip_info_old);
        g_h.funcs->_h_free(esp_netif->ip_info);
        g_h.funcs->_h_free(esp_netif);
        return NULL;
    }
...
static esp_err_t esp_netif_lwip_add(esp_netif_t *esp_netif)
{
    if (esp_netif->lwip_netif == NULL) {
        esp_netif->lwip_netif = g_h.funcs->_h_calloc(1, sizeof(struct netif));
        if (esp_netif->lwip_netif == NULL) {
...
void esp_netif_destroy(esp_netif_t *esp_netif)
{
    if (esp_netif) {
        ...
        g_h.funcs->_h_free(esp_netif->ip_info);
        g_h.funcs->_h_free(esp_netif->ip_info_old);
        g_h.funcs->_h_free(esp_netif->if_key);
        g_h.funcs->_h_free(esp_netif->if_desc);
        esp_netif_lwip_ipc_call(esp_netif_lwip_remove_api, esp_netif, NULL);
        esp_netif_destroy_related(esp_netif);
        g_h.funcs->_h_free(esp_netif->lwip_netif);
        g_h.funcs->_h_free(esp_netif->hostname);
    ...
    if (esp_netif->flags & ESP_NETIF_DHCP_SERVER) {
        ...
        if (esp_netif->dhcpc_status != ESP_NETIF_DHCP_STARTED) {
            if (p_netif != NULL) {
                ...
                if (dhcp_data == NULL) {
                    dhcp_data = (struct dhcp *)g_h.funcs->_h_malloc(sizeof(struct dhcp));
                    if (dhcp_data == NULL) {
                        return ESP_ERR_NO_MEM;
                    }
