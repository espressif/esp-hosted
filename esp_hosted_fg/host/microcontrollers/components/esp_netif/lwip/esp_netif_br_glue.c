...
{
    esp_netif_br_glue_t *netif_glue = g_h.funcs->_h_calloc(1, sizeof(esp_netif_br_glue_t));
    if (!netif_glue) {
...
esp_err_t esp_netif_br_glue_add_port(esp_netif_br_glue_handle_t netif_br_glue, esp_netif_t *esp_netif_port)
{
    if (netif_br_glue->ports_esp_netifs == NULL) {
        netif_br_glue->ports_esp_netifs = g_h.funcs->_h_nw_malloc(sizeof(esp_netif_t *));
    } else {
...
esp_err_t esp_netif_br_glue_del(esp_netif_br_glue_handle_t netif_br_glue)
{
    ...
    g_h.funcs->_h_free(netif_br_glue->ports_esp_netifs);
    g_h.funcs->_h_free(netif_br_glue);
    netif_br_glue = NULL;
    return ESP_OK;
}
