...
static void display_arp(uint8_t *pkt, uint16_t pkt_len, uint8_t arp_msg)
{
	...
	if (arp_msg < MAX_MSG_TYPE_ARPING) {
		...
		printf("0x%05lX %s: %u bytes from %s (%s)\n\r",
				(long unsigned int) ++count, str_req[arp_msg-1], pkt_len,  ip_addr_s, mac_s);
	}
}
		...
		uint8_t *pkt, uint16_t pkt_len, uint16_t *resp_len)
{
	...
	arp_resp = g_h.funcs->_h_malloc(ARPING_MAX_PKT_SIZE);
	assert(arp_resp);


	/* replicate packet */
	g_h.funcs->_h_memset(arp_resp, 0, ARPING_MAX_PKT_SIZE);
	g_h.funcs->_h_memcpy(arp_resp, pkt, ARPING_MAX_PKT_SIZE);
		...
		uint32_t *src_ip, uint8_t *dst_mac, uint32_t *dst_ip)
{
	...
	buffer->payload = g_h.funcs->_h_malloc(ARPING_MAX_PKT_SIZE);
	assert(buffer->payload);

	buffer->len = 42;

	g_h.funcs->_h_memcpy(buffer->payload, arp_req_tx, ARPING_MAX_PKT_SIZE);
