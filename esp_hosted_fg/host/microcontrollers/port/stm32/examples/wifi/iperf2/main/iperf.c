...
#define ENABLE_IPERF_IPV6 LWIP_IPV6
typedef struct {
    iperf_cfg_t cfg;
    bool finish;
    uint32_t actual_len;
...
#define IPERF_TEST_DIVISOR 1000 /*1e6*/
#else
    uint32_t average = 0;
    uint32_t actual_bandwidth = 0;

#define IPERF_TEST_DIVISOR 1000
#endif
    int k = 1;
    ...
    while (!s_iperf_ctrl.finish) {
        ...
        actual_bandwidth = (s_iperf_ctrl.actual_len / IPERF_TEST_DIVISOR * 8) / interval;
#if FLOATING_SUPPORT
        ESP_LOGI(TAG, "%4d-%4d sec       %.2f kbps", cur, cur + interval,
            actual_bandwidth);
#else
        ESP_LOGI(TAG, "%4d-%4d sec       %u kbps", cur, cur + interval,
                    actual_bandwidth);
#endif
        cur += interval;
        ...
        if (cur >= time) {
#if FLOATING_SUPPORT
        	ESP_LOGI(TAG, "%4d-%4d sec       %.2f kbps", 0, time, average);
#else
        	ESP_LOGI(TAG, "%4d-%4d sec       %u kbps", 0, time, average);
#endif
        	break;
        }
    }
...
static void socket_send(int send_socket, struct sockaddr_storage dest_addr, uint8_t type, int bw_lim)
{
    ...
    if (bw_lim > 0) {
        period_ms = want_send * 8 / bw_lim * 1000;
    }
    ...
    while (!s_iperf_ctrl.finish) {
        if (period_ms > 0) {
            ...
            if (actual_send > 0){
                ...
                // next send delay.
                delay_ms += period_ms + (int32_t)(prev_time - send_time);
            } else {
...
esp_err_t iperf_start(iperf_cfg_t *cfg)
{
    ...
    s_iperf_ctrl.buffer = (uint8_t *)g_h.funcs->_h_malloc(s_iperf_ctrl.buffer_len);
    if (!s_iperf_ctrl.buffer) {
