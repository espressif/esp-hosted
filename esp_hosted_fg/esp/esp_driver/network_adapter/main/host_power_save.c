...
int is_host_wakeup_needed(interface_buffer_handle_t *buf_handle)
{
	...
	switch (buf_handle->if_type) {
		...
		case ESP_STA_IF:
			 ...
			 * *selectively* filter packets on criterias which can decide if packet should be :
			 * a. consumed locally at coprocessor OR
			 * b. drop OR
			 * c. fowarding it to host
			 *
			 * Network Split criteria (if enabled) apllied before reaching her.
			 **/
